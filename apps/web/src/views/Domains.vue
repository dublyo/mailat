...
function downloadDNSZoneFile(domain: any) {
  ...
  const lines: string[] = [
    ...
    `; Generated by Mailat - ${new Date().toISOString().split('T')[0]}`,
    `; Import this file into Cloudflare:`,
  ...
  for (const record of records) {
    const hostname = (record.hostname || record.name || '').replace(/\.$/, '')
    const type = record.recordType || record.type || ''
    const value = record.value || record.expectedValue || ''

    if (type === 'MX') {
      // MX format: "10 mail.example.com" -> priority + host
      const parts = value.match(/^(\d+)\s+(.+)$/)
      if (parts) {
        lines.push(`${hostname}.\t1\tIN\tMX\t${parts[1]}\t${parts[2]}.`)
      } else {
        lines.push(`${hostname}.\t1\tIN\tMX\t10\t${value}.`)
      }
    } else if (type === 'CNAME') {
      const target = value.replace(/\.$/, '')
      lines.push(`${hostname}.\t1\tIN\tCNAME\t${target}.`)
    } else if (type === 'TXT') {
      lines.push(`${hostname}.\t1\tIN\tTXT\t"${value}"`)
    } else {
      lines.push(`${hostname}.\t1\tIN\t${type}\t"${value}"`)
    }
  }
              ...
              <div v-if="expandedDomains.has(domain.uuid)" class="border-t border-gray-100">
                ...
                <div class="p-5 bg-gray-50">
                  <div class="flex items-center justify-between mb-3">
                    ...
                    <button
                      v-if="domain.dnsRecords && domain.dnsRecords.length > 0"
                      @click="downloadDNSZoneFile(domain)"
                      class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-orange-700 bg-orange-50 hover:bg-orange-100 border border-orange-200 rounded-lg transition-colors"
                      title="Download zone file for Cloudflare import"
                    >
                      <Download class="w-3.5 h-3.5" />
                      Download for Cloudflare
                    </button>
                  </div>
            ...
            <div v-if="showSetupWizard" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div class="flex-1 overflow-y-auto p-6">
                ...
                <div v-if="setupStep === 2">
                  ...
                  <!-- Download Zone File -->
                  <div v-if="selectedDomain?.dnsRecords && selectedDomain.dnsRecords.length > 0" class="mb-4">
                    <button
                      @click="downloadDNSZoneFile(selectedDomain)"
                      class="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-orange-700 bg-orange-50 hover:bg-orange-100 border border-orange-200 rounded-xl transition-colors"
                    >
                      <Download class="w-4 h-4" />
                      Download DNS Zone File (Cloudflare Import)
                    </button>
                    <p class="text-xs text-gray-500 mt-2 text-center">
                      Go to Cloudflare > DNS > Records > Import and Export > Import DNS records
                    </p>
                  </div>
